...
meanStdIndices <- grep("mean\\(\\)|std\\(\\)", names(mergedSet))
mergedSet <- mergedSet[,names(mergedSet)[meanStdIndices]]
dim(mergedSet)

# Task 3
# Add Subject and Activity column in the merged data set
...
# Task 5
# Creates an independent tidy data set with the average of
# each variable for each activity and each subject 
tidy <- aggregate(. ~ Subject + Activity, data=mergedSet, mean)
# Write tidy data set as a txt file
